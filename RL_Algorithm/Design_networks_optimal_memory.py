...
import torch
import numpy as np
from RL_Algorithm.model.PPO_optimal_memory import PPO
...
def evaluate_policy(args, env, agent,virtual_node_g,add_edges=100,):
    ...
    with torch.no_grad():
        for id_ in graph_ids:
            ...
            agent.actor_net.reset_connected_edges(G0)
            G = copy.deepcopy(G0)
            G_list = [G0]
            for t in count():
                state = virtual_node_g(G)
                act = agent.select_action(state,evaluation=True)
                ...
                if len(G.edges()) >= len(G0.edges)+add_edges:
                    ...
                    break
                agent.actor_net.update_connected_edges(act)
