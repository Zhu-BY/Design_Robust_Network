...
def test(args, model, networks_provider=None, print_model=True, print=print):
    ...
    with torch.no_grad():
        ...
        # for filename, network, data in ProgressBar()(networks_provider,redirect_stdout=False):
        for filename, network, data in tqdm(networks_provider,disable=True):
...
def train_wrapper(args, nn_model, train_ne=True, networks_provider=None, print=print):
    ...
    if model_weights_file.is_file():
        ...
        model.load_state_dict(torch.load(str(model_weights_file),map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu"),weights_only=True), strict=False)
        # model.load_state_dict(torch.load(str(model_weights_file),map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu")), strict=False)
