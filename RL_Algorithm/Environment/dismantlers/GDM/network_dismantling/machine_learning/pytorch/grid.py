...
def init_network_provider(location, targets, features_list, filter="*", callback=None,id=0):
    test_networks = storage_provider(location, filter=filter, callback=callback,id=id)
...
# 修改以适配networkx.graph
from network_dismantling.machine_learning.pytorch.dataset_providers import storage_provider_nx
def init_network_provider_nx(G_nx, location, targets, features_list, filter="*", callback=None):
    test_network = storage_provider_nx(G_nx,location, filter=filter, callback=callback)
    ...
    for features in features_list:
        ...
        # TODO REMOVE THIS LIST
        # pp_test_network[key] = list(zip(tuple('test_nx'), tuple(test_network), map(lambda n: prepare_graph(n, features=features, targets=targets), tuple(test_network))))
        pp_test_network[key] = [('test_nx',test_network,prepare_graph(test_network,features=features, targets=targets))]        

    return pp_test_network
