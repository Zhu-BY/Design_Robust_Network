...
string FILE_NET;  // input the network, format: id id
string FILE_ID;   // input the id of the removed nodes before reinserting
string FILE_ID2;   // output the id of the removed nodes after reinserting
int Sort_Strategy=2;
}
...
po::variables_map parse_command_line(int ac, char ** av)
{
	...
	desc.add_options()
		...
		("threshold,t", po::value(&threshold)->default_value(threshold),  // threshold
		 "stop on threshold")
		("FILE_NET,N", po::value(&FILE_NET),"input the network")
		("FILE_ID,I", po::value(&FILE_ID),"input the id of the removed nodes before reinserting")
		("FILE_ID2,D", po::value(&FILE_ID2),"output the id of the removed nodes after reinserting")
		("Sort_Strategy,S", po::value(&Sort_Strategy), "removing order");// removing order
//							 // 0: keep the original order
//							 // 1: ascending order - better strategy for weighted case
//							 // 2: descending order - better strategy for unweighted case
