...
from RL_Algorithm.model.net_optimal_memory import Actor,Critic
...
class PPO():
    ...
    def select_action(self, state,evaluation = False,returnall=False):
        device = self.actor_net.fc1.weight.device
        with torch.no_grad():
            action,score = self.actor_net(state.to(device),state.ndata['feat'].to(device))
            return action
    ...
    def update(self, i_ep):  # 多个trajectory训练
        ...
        for i in range(self.ppo_update_time):
            ...
            # 全部結果的action loss和value loss
                    # 分批计算全部结构的action loss和value loss
        with torch.no_grad():
