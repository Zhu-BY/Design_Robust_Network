...
import numpy as np
import time
from RL_Algorithm.utils.rl_utils import graph_batch,graph_batch_with_cost
import dgl
from RL_Algorithm.model.net_cost import Actor_cost,Critic_cost
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

class PPO_cost():
    def __init__(self, args,creat_file = True):
        super(PPO_cost, self).__init__()
        ...
        self.actor_net = Actor_cost(args).to(device)
        self.critic_net = Critic_cost(args).to(device)
        self.buffer = []
        self.counter = 0
        ...
        if creat_file:
            if not os.path.exists('./RL_Algorithm/Trained_models/random_cost/cost_%s_%s_%s_%s/param' % (args.dismantling_name,args.graph_type,args.num_nodes,self.nowtime)):
                os.makedirs('./RL_Algorithm/Trained_models/random_cost/cost_%s_%s_%s_%s/param/net_param' % (args.dismantling_name,args.graph_type,args.num_nodes,self.nowtime))
                os.makedirs('./RL_Algorithm/Trained_models/random_cost/cost_%s_%s_%s_%s/param/img' % (args.dismantling_name,args.graph_type,args.num_nodes,self.nowtime))
            self.path ="./RL_Algorithm/Trained_models/random_cost/cost_%s_%s_%s_%s"% (args.dismantling_name,args.graph_type,args.num_nodes,self.nowtime)

    def select_action(self, state,edge_mask_matrix,normalize_cost,evaluation = False,returnall=False):
        ...
        with torch.no_grad():
            action_prob = self.actor_net(state.to(device),state.ndata['feat'].to(device),edge_mask_matrix,normalize_cost)
        if evaluation==True:
        # if True:
            if returnall==False:
    ...
    def update(self, i_ep):  # 多个trajectory训练
        ...
        normalize_cost = [t.state[2] for t in self.buffer]

        old_action_log_prob = torch.tensor([t.a_log_prob for t in self.buffer], dtype=torch.float).view(-1, 1).to(device)

        batch_state, batch_state_features, batch_edge_mask_matrixes,batch_normalize_cost = graph_batch_with_cost(state, edge_mask_matrixes,normalize_cost)
        ...
        #     R = r + self.gamma * R
        #     Gt.insert(0, R)
        # Gt = torch.tensor(Gt, dtype=torch.float) # 所有的Gt
        # # 所有的advantage
        # V = self.critic_net(batch_state.to(device), batch_state_features.to(device))
        # delta = Gt.view(-1,1).to(device) - V
        # batch_advantage = delta.detach().squeeze()
        # # advantage normalization (batch adv)
        # mean_adv = batch_advantage.mean()
        # std_adv = batch_advantage.std()
        # norm_adv = (batch_advantage - mean_adv) / std_adv
        ...
        for i in range(self.ppo_update_time):
            for index in BatchSampler(SubsetRandomSampler(range(len(self.buffer))), self.mini_batch_size, False):
                ...
                batch_state,batch_state_features,batch_edge_mask_matrixes,batch_normalize_cost=graph_batch_with_cost([state[ind] for ind in index],[edge_mask_matrixes[ind] for ind in index],[normalize_cost[ind] for ind in index])
                V = self.critic_net(batch_state.to(device),batch_state_features.to(device))
                if self.training_step>self.value_first:
                    advantage = norm_adv[index].view(-1,1).to(device)
                # epoch iteration, PPO core!!!
                    action_probs = self.actor_net(batch_state.to(device), batch_state_features.to(device),batch_edge_mask_matrixes,batch_normalize_cost)  # new policy
                ...
                #     print('I_ep {} ，train {} times , action loss {:.4f} , value loss {:.4f}'.format(i_ep, self.training_step,action_loss.mean(),value_loss))
        # 全部結果的action loss和value loss
        with torch.no_grad():
            ...
            for i in range(num_batch):
                ...
                batch_state, batch_state_features, batch_edge_mask_matrixes,batch_normalize_cost= graph_batch_with_cost(state[i0:i1],edge_mask_matrixes[i0:i1],normalize_cost[i0:i1])
                V = self.critic_net(batch_state.to(device), batch_state_features.to(device))
