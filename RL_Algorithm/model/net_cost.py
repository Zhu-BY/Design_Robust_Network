...
class Actor_cost(nn.Module):  # 图神经网络_no batch
    """docstring for Net"""
    def __init__(self, args):
        super(Actor_cost, self).__init__()
        ...
        if args.gnn_type == 'GCN':
            ...
            for _ in range(args.num_layers):
                self.gcn_layers.append(
                    GraphConv(args.hidden_dim1, args.hidden_dim1,bias=True,weight=True,allow_zero_in_degree=True))
        ...
        self.cost_fn = nn.Linear(1,int(args.hidden_dim1*0.5))

        # 第二、三个全连接层
        if self.graph_embed:
            self.fc2 = nn.Linear(int(args.hidden_dim1*3.5), args.hidden_dim2)
        else:
            self.fc2 = nn.Linear(int(args.hidden_dim1 * 2.5), args.hidden_dim2)
        ...
        #         B.append(b)
        #         C.append(c)
        # self.node_to_edge_matrix1 = torch.tensor(B,dtype=torch.float32).to(device)
        # self.node_to_edge_matrix2 = torch.tensor(C,dtype=torch.float32).to(device)
    ...
    def forward(self, g, node_features,edge_features_mask_matrix,normalize_cost,pre_train=False):  # g:DGLGraph.batch  node_features:batch_size,节点数,时间序列   edge_features:batch_size,边数,值
        if len(node_features.shape) == 3:  # 批量输入
            ...
            # 边embedding添加边的标准化程度
            batch_normalize_cost = normalize_cost.reshape(batch_size,-1,1).float()
            batch_normalize_cost_ = self.cost_fn(batch_normalize_cost)
            batch_emb_edge = torch.concat((batch_emb_edge,batch_normalize_cost_),dim=2)
            # 添加结束##########
            if self.graph_embed:
            ...
            action_prob = torch.where(batch_edge_features_mask_matrix,
                                      action_prob,
                                      action_null)
            ###################action mask#######################
            action_prob = action_prob.double()
            if pre_train:return action_prob
            try:
            ...
            # 边embedding添加边的标准化程度
            normalize_cost = normalize_cost.reshape(-1,1).float()
            normalize_cost_ = self.cost_fn(normalize_cost)
            emb_edge = torch.concat((emb_edge,normalize_cost_.float()),dim=1)
            # 添加结束
            ################## 屏蔽已有边###################
            emb_edge_null = torch.zeros(emb_edge.shape).to(device)
            emb_edge = torch.where(
                torch.tile(edge_features_mask_matrix.unsqueeze(-1), (1, emb_edge.shape[-1])),
                emb_edge,
                emb_edge_null)
            # 全连接回归层
...
class Critic_cost(nn.Module):  # 图神经网络_no batch
    """docstring for Net"""
    def __init__(self, args):
        super(Critic_cost, self).__init__()
        if args.gcn_activation =='relu': self.gcn_activation = nn.ReLU()
        if args.gcn_activation == 'elu': self.gcn_activation = nn.ELU()
        if args.gcn_activation == 'tanh': self.gcn_activation = nn.Tanh()
        try:
